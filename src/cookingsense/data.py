...
def load_raw_dataset(domain, config):
    if domain == "web":
        web_raw_dataset = load_dataset(
            "allenai/c4", "en", streaming=config.data_streaming
        )["train"]
        ...
        paper_dataset = Dataset.from_pandas(
            pd.DataFrame(paper_df["title"] + " " + paper_df["tldr"], columns=["text"])
        )
        ...
        recipe_raw_dataset = load_dataset(
            "recipe_nlg", data_dir=config.data_dir, streaming=config.data_streaming
        )
        return recipe_raw_dataset


def load_datasets(config):
    return {k: load_raw_dataset(k, config) for k in __domains__}
